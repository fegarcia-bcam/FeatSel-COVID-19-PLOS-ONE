...
PATH = config.PATH_RESULTS_WRA_RFECV
...
if __name__ == '__main__':
    ...
    for file_in in files:
        ...
        if estimator == 'LogReg':
            ...
            if imputer == 'knn':
                ...
                label = 'RFECV Regress (knn)'
            elif imputer == 'iterative':
                col = 'Regression (Ridge) + iterative'
                label = 'RFECV Regress (iter)'
            else:
                raise RuntimeError
        ...
        df_stab_m[col] = stab_mean
        df_stab_ci[col] = stab_ci_d
        df_time_m[col] = t_mean
        df_time_ci[col] = t_ci_d
    ...
    path_fig = os.path.join(config.PATH_FIGURES, 'Stability_Wrapper [RFECV].{}'.format(IMG_FORMAT))
    ...
    path_fig = os.path.join(config.PATH_FIGURES, 'Time_Wrapper [RFECV].{}'.format(IMG_FORMAT))
    fig.savefig(path_fig, format=IMG_FORMAT, dpi=IMG_DPI, pil_kwargs=IMG_EXTRA)
