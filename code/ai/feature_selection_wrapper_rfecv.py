...
# feature selection
FEATURE_SELECTOR = 'wrap-rfecv'

# feature selection parameters
STEP = 3

FEAT_SELECTORS = [{'analysis': 'classif',
                   'selector_params': {'estimator': 'LogReg', 'step': STEP, 'min_features_to_select': STEP, 'cv': config.N_CV_SPLITS}},
                  {'analysis': 'regress',
                   'selector_params': {'estimator': 'Ridge', 'step': STEP, 'min_features_to_select': STEP, 'cv': config.N_CV_SPLITS}}]
            ...
            for imp in IMPUTERS:
                ...
                if config.PARALLEL:
                    ...
                    f_partial_c = partial(feat_sel_classif,
                                          ...
                                          scaler=SCALER,
                                          imputer=imputer, imputer_params=imputer_params,
                                          balancer=BALANCER, balancer_params=BALANCER,
                ...
                f_results = os.path.join(config.PATH_RESULTS_WRA_RFECV, filename)
