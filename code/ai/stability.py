...
def getVarianceofStability(Z):
    ...
    OUTPUT: A dictionary where the key 'stability' provides the corresponding stability value #
...
def confidenceIntervals(Z, alpha=config.ALPHA, res=None):
    """
    Let us assume we have M>1 feature sets and d>0 features in total.
    ...
    INPUTS: - A BINARY matrix Z (given as a list or as a numpy.ndarray of size M*d, raises a ValueError exception otherwise).
            ...
            - In case you already computed the stability estimate of Z using the function getVarianceofStability(Z),
              you can provide the result (a dictionary) as an optional argument to this function for faster computation.

    OUTPUT: - A dictionary where the key 'stability' provides the corresponding stability value, where:
    ...
    if len(res) == 0:
        res = getVarianceofStability(Z)  # get a dictionary with the stability estimate and its variance
...
def hypothesisTestV(Z, stab0, alpha=config.ALPHA):
    """
    Let us assume we have M>1 feature sets and d>0 features in total.
...
def hypothesisTestT(Z1, Z2, alpha=config.ALPHA):
    """
    Let us assume we have M>1 feature sets and d>0 features in total.
...
def checkInputType(Z):
    """
    This function checks that Z is of the right type and dimension.
