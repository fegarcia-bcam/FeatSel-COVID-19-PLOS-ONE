...
PATH = config.PATH_RESULTS_WRA_SFS
...
if __name__ == '__main__':
    ...
    for file_in in files:
        ...
        if estimator == 'LogReg':
            if imputer == 'knn':
                ...
                label = 'SFS Classif[LogReg] (knn) NFS {:02d}'.format(n_feat)
            elif imputer == 'iterative':
                col = 'Classification (LogReg) + iterative'
                label = 'SFS Classif[LogReg] (iter) NFS {:02d}'.format(n_feat)
            else:
                raise RuntimeError
            ...
            if imputer == 'knn':
                ...
                label = 'SFS Regress[Ridge] (knn) NFS {:02d}'.format(n_feat)
            elif imputer == 'iterative':
                col = 'Regression (Ridge) + iterative'
                label = 'SFS Regress[Ridge] (iter) NFS {:02d}'.format(n_feat)
            else:
                raise RuntimeError
            ...
            if imputer == 'knn':
                ...
                label = 'SFS Classif[KNN] (knn) NFS {:02d}'.format(n_feat)
            elif imputer == 'iterative':
                col = 'Classification (KNNC) + iterative'
                label = 'SFS Classif[KNN] (iter) NFS {:02d}'.format(n_feat)
            else:
                raise RuntimeError
            ...
            if imputer == 'knn':
                ...
                label = 'SFS Regress[KNN] (knn) NFS {:02d}'.format(n_feat)
            elif imputer == 'iterative':
                col = 'Regression (KNNR) + iterative'
                label = 'SFS Regress[KNN] (iter) NFS {:02d}'.format(n_feat)
            else:
                raise RuntimeError

        elif estimator == 'HGBC':
            col = 'HGB Classification (HGBC)'
            label = 'SFS Classif[HGB] NFS {:02d}'.format(n_feat)

        elif estimator == 'HGBR':
            col = 'HGB Regression (HGBR)'
            label = 'SFS Regress[HGB] NFS {:02d}'.format(n_feat)
    ...
    path_fig = os.path.join(config.PATH_FIGURES, 'Stability_Wrapper [SFS].{}'.format(IMG_FORMAT))
    ...
    path_fig = os.path.join(config.PATH_FIGURES, 'Time_Wrapper [SFS].{}'.format(IMG_FORMAT))
    fig.savefig(path_fig, format=IMG_FORMAT, dpi=IMG_DPI, pil_kwargs=IMG_EXTRA)
